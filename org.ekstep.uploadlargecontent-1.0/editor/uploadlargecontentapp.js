...
angular.module('org.ekstep.uploadlargecontent-1.0', []).controller('largeUploadController', ['$scope', '$injector', 'instance', '$timeout', function ($scope, $injector, instance, $timeout) {
    ...
    $scope.fileValidation = function () {
        ...
        $scope.mimeType = ($scope.uploader.getFile(0) != null) ? $scope.detectMimeType($scope.uploader.getName(0), $scope.isScorm) : '';
        if (!$scope.mimeType) {
        ...
        $scope.contentService.getPresignedURL(ecEditor.getContext('contentId'), $scope.uploader.getName(0), function (err, res) {
            ...
            if (err) {
                ...
                if (cloudstorage != null || typeof cloudstorage != "undefined") {
                    ...
                    const contentType = ($scope.uploader.getFile(0) != null) ? $scope.detectMimeType($scope.uploader.getName(0), $scope.isScorm) : '';
