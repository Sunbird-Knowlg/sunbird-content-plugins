...
angular.module('videoApp', [])
    .controller('videoCtrl', ['$scope', '$injector', 'instance', function ($scope, $injector, instance) {
        ...
        ctrl.maxVideoSize = ecEditor.getContext('MAX_ASSET_FILE_SIZE_LIMIT');
        ctrl.previewMessages = {
            ...
            fileExceed: 'Video file is exceeded'
        }
...
// Video js uses CSS3 query selector which does not allow #IDs to start with non alphabets
        ...
        ctrl.driveLoader =  function(videoelement, video){
            ...
            org.ekstep.contenteditor.api.getService(ServiceConstants.META_SERVICE).getDriveVideoFileSize(requestObj, function (err, res) {
                if (err) {
                    ctrl.toastManager('error', true, false, ctrl.previewMessages.invalidDriveURL);
                    ctrl.isPreviewPlaying = false;
                }else if(res.data.result.size.valid && res.data.result.size.value <= ctrl.maxVideoSize) {
                    video.play()
                        .then(function () {
                            ctrl.isPreviewPlaying = true;
                            ctrl.toastManager('', false, true, '');
        ...
        ctrl.bytesToMB = function(){
            var sizes = ['Bytes', 'KB', 'MB', 'GB', 'TB'];
            if (ctrl.maxVideoSize == 0) return '0 Byte';
            var i = parseInt(Math.floor(Math.log(ctrl.maxVideoSize) / Math.log(1024)));
            ctrl.videoSizeinMB = Math.round(ctrl.maxVideoSize / Math.pow(1024, i), 2) + ' ' + sizes[i];
        };
