    ...
    initPlugin: function(data) {
        ...
        if (value && _.isArray(value) && value.length > 0) {
            if (layout === 'table' && (_.isFinite(data.cols) || _.isFinite(data.rows))) {
                this._parent._controller.setModelValue(model,this._registerResValues(value));
        ...
        for (var r=0; r<rows; r++) {
            for (var c=0; c<cols; c++) {
                if (c*r < count) {
                    ...
                    index = index + 1;
                    PluginManager.invoke('coption', data, instance._parent, instance._stage, instance._theme);
                }
            }
        }
