...
describe("compatibility check `Topic selector-1.0` with ` Editor 3.2.0`", function() {
    ...
    describe('Plugin initialization', function() {
        it('Should open the popup, When categories are present', function(done) {
            ...
            //          
            var data = {"element":"defaultTemplate-topic","selectedTopics":[],"isCategoryDependant":true}
            pluginInstance.getTopics = jasmine.createSpy().and.callFake(function(data, callback) {
                instance.categories = [{ id: "topic1" }];
            });
        ...
        it('Should show an popup and throw an error message, When categories are not present', function(done) {
            var data = {};
            //  assertions
            //         - POPUP should open
    ...
    describe('applyFilters', function() {
        it('When valid object is passed, Should topic browser', function(done) {
            var data = {};
            ecEditor.addEventListener('editor:form:change', function(event, data) {
                expect(data.key).toBe('topic');
                expect(data.value).not.toBe(undefined);
            })
            ecEditor.addEventListener('metadata:form:getformdata', function(event, data) {
                expect(data.key).toBe('topic');
                expect(data.value).not.toBe(undefined);
            })
        ...
        it('When Invalid object is passed, Should throw an error', function(done) {
            try {
                var data = {};
                pluginInstance.applyFilters('', undefined);
                expect(pluginInstance.selectedFilters).not.toBe(undefined);
                done()
            } catch (e) {
                expect(e).not.toBe(undefined);
                expect(e).toBe('Invalid filters');
                done()
