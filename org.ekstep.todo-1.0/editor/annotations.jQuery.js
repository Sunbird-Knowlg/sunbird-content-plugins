...
$.fn.annotations = function(options){
	...
	function loadComments(element) {
		ecEditor.jQuery(element).comments({
			...
			// Fields mapping
			fieldMappings: { id: 'annotation_id', parent: 'parent_id', content: 'annotation', content_html: 'annotation_html', created: 'annotation_date', fullname: 'user_name', createdByCurrentUser: 'is_mine',
			},
					...
					success:function(result) {
						...
						if (result.data.total > limit) {
							...
							else
							{
								...
								jQuery(".data-container", element).append(loadMoreBtn); ecEditor.jQuery('.cmt_loader').hide();
