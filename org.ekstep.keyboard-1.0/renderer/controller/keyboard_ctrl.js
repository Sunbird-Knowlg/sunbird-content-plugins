...
app.controllerProvider.register("KeyboardCtrl", function($scope) {
  ...
  EkstepRendererAPI.addEventListener("renderer:keyboard:invoke", function(e, callback) {
    ...
    if ($scope.config.question.keyboardConfig.keyboardType == 'English') {
      ...
      $("#parentDivMainKeyboard").length ? "" : $scope.createKeyboard(customButtons);
  ...
  $scope.createKeyboard = function(customButtons) {
    ...
    if (type == "custom") {
      ...
      if(buttons.length > 10){
        eachRowLength = parseInt(buttons.length / 2);
      }else{
        eachRowLength = parseInt(buttons.length);
      }
  ...
  $scope.addLetter = function(event) {
    if (!_.isUndefined($scope.answerText)) {
      if (event.target.innerText != '123')
        $scope.answerText = $scope.answerText + event.target.innerText;
    } else {
      if (event.target.innerText != '123')
